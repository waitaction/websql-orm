...
export class Sql {
     ...
     */
    fromSql<T extends Table>(tableInstance: T, sql: string, value: Array<any>): Promise<Array<T>> {
     ...
     * @param value sql参数值,例如 ['Tom','Girl']
     */
    async fromSqlFirst<T extends Table>(tableInstance: T, sql: string, value: Array<any>): Promise<T> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     */
    async exist<T extends Table>(tableInstance: T, primaryValue: string): Promise<boolean> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     * @param primaryValue 主键值
     */
    async delete<T extends Table>(tableInstance: T, primaryValue: string): Promise<boolean> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     */
    async insert<T extends Table>(value: T | Array<T>): Promise<number> {
     ...
     * @param value 定义的实体实例数据，例如 var stu = new student(); stu.user_name = 'Tom' ...
     */
    async update<T extends Table>(value: T): Promise<number> {
        value.checkColDef();
        var context = new DbContext<T>(<any>value.constructor);
     ...
     */
    async execSql<T extends Table>(tableInstance: T, sql: string, value: Array<number | string | Date | boolean | any>): Promise<number> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     */
    async query<T extends Table>(tableInstance: T, param: any): Promise<Array<T>> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     * @param param json对像参数，例如 { id:'e453c1e4-b7fc-4775-a2b4-26a0d834e83d' , user_name:'Tom' } , 类似于 where id='e453c1e4-b7fc-4775-a2b4-26a0d834e83d' and user_name='Tome'
     */
    async queryFirst<T extends Table>(tableInstance: T, param: any): Promise<T> {
        var context = new DbContext<T>(<any>tableInstance.constructor);
     ...
     * @param value 定义的实体实例数据，例如 var stu = new student(); stu.user_name = 'Tom' ...
     */
    async save<T extends Table>(value: T): Promise<number> {
        var context = new DbContext<T>(<any>value.constructor);
